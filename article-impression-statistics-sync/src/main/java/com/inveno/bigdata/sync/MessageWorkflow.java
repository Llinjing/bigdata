...
public class MessageWorkflow extends AbstractMessageProcessor {
	
	private static final Logger logger = LoggerFactory.getLogger(MessageWorkflow.class);
	...
	public MessageWorkflow(AbstractConverter converter) {
	...
	public boolean init() {
		...
		for (int i = 0; i < inner_queue_num; ++i) {
			...
			// kafka writer
			MessageKafkaWriter kafka_writer = new MessageKafkaWriter(config.getConfig().getString("src.kafka.zk.list"),
					config.getConfig().getString("src.kafka.broker.list"));
