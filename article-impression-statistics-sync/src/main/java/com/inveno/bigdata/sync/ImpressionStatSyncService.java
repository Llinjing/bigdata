...
import com.inveno.bigdata.sync.convert.AbstractConverter;
import com.inveno.bigdata.sync.convert.ConverterFactory;


public class ImpressionStatSyncService implements Lifecycleable {
	
	private static final Logger logger = LoggerFactory.getLogger(ImpressionStatSyncService.class);
	
	private static final String CLASS_NAME = ImpressionStatSyncService.class.getSimpleName();
		...
		if (topic_partition != null) {
			...
			if (arr != null && arr.length == 2) {
				...
				for (int i = 0; i < partitions; ++i) {
					...
					MessageWorkflow workflow = new MessageWorkflow(converter);
	...
	public static void main(String [] args) {
		...
		ImpressionStatSyncConfig config = ImpressionStatSyncConfig.getInstance();
		...
		ImpressionStatSyncService service = new ImpressionStatSyncService();
		if (service.init()) {
			logger.info("{} init ok", CLASS_NAME);
			
			Runtime.getRuntime().addShutdownHook(new ServiceShutdownHook(service));
		} else {
