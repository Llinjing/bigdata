...
object FeedBackUtil extends LogSupport with InfoProcess {
  ...
  def usingJson(op: JSONObject => String) = {
    ...
    try {
      ...
      case e: Exception => log.error(s"FeedBackUtil: => failed to get json results ! Json info ${json.toString}", e.printStackTrace())
        ""
    }
  }
