...
object AdvertiseFeedback extends LogSupport with InfoProcess {
  ...
  def updateRedis(dataSet: DStream[(String, (Long, Long, Long))], flag: Boolean = true): Unit = {
    try {
      ...
      val hash_key = NearLineConfig.redisGateInfo.hashkey
      dataSet.foreachRDD(rdd => {

        rdd.foreachPartition(partition => {
          try {

            jdClient = if (flag) new Jedis(NearLineConfig.redisMalaccaInfo.server, NearLineConfig.redisMalaccaInfo.port)
            else new Jedis(NearLineConfig.redisGateInfo.server, NearLineConfig.redisGateInfo.port)

            jdClient.connect()
            if (jdClient != null) {

              partition.foreach((info: (String, (Long, Long, Long))) => {
                val fields = info._1.split(Constant.KEY_TAG)
