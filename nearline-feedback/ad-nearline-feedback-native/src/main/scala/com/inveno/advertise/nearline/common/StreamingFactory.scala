...
import com.inveno.advertise.nearline.zookeeper.ZooKeeperUtil
...
object StreamingFactory extends LogSupport {
  ...
  def createDirectStreamByOffset(zk_hosts: String,
    ...
    val last_offsets = ZooKeeperUtil.readOffset(zk_hosts,
      offset_zk_path,
    ...
    for (offset <- last_offsets) {
      ...
      log.info("start from, topic = " + offset.topic +
        ", partition = " + offset.partition +
        ", start_offset = " + offset.untilOffset)
    }

    val kafka_stream = if (from_offsets.isEmpty) {
      KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder](ssc, kafka_param, Set(topic))
    } else {
      val msg_handler = (mmd: MessageAndMetadata[String, String]) => (mmd.topic, mmd.message)
      KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder, (String, String)](ssc, kafka_param, from_offsets, msg_handler)
    }

    save_offset(kafka_stream, zk_hosts,
      offset_zk_path,
      kafka_param.get("group.id").get)
  ...
  def save_offset(kafka_stream: InputDStream[(String, String)],
    ...
    kafka_stream.foreachRDD(rdd => {
      ...
      for (offset <- offsetArr) {
        val ret = ZooKeeperUtil.writeOffset(zk_list, offset_zk_path, consumer_group, offset)
        if (!ret) {
          log.warn("write offset failed : group_id = " + consumer_group + ", offset = " + offset)
        } else {
          log.info("write offset succeed : group_id = " + consumer_group + ", offset = " + offset)
        }
      }
    })
  }
