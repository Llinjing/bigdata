...
object StatUserClick {
  ...
  def click(clickInput: String, sc: SparkContext): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
  ...
  def processPartition(iter: Iterator[String]): Iterator[(String, (Long, LinkedList[String]))] = {
    ...
    while (iter.hasNext) {
      val str = iter.next
      val data = ReformatDataParser.parseClick(str, set)
      if (!data.isEmpty) {
        val arr = data.get
