...
object StatUserListpageDwelltime {
  ...
  def listpageDwelltime(listpageDwelltimeInput: String, sc: SparkContext): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
  ...
  def processPartition(iter: Iterator[String]): Iterator[(String, (Long, LinkedList[String]))] = {
    ...
    while (iter.hasNext) {
      val str = iter.next
      val data = ReformatDataParser.parseListpageDwelltime(str, set)
      if (!data.isEmpty) {
        val arr = data.get
