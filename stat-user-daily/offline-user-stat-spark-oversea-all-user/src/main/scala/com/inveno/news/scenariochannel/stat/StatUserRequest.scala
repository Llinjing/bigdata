...
object StatUserRequest {
  ...
  def request(requestInput: String, sc: SparkContext): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
  ...
  def processPartition(iter: Iterator[String]): Iterator[(String, (Long, LinkedList[String]))] = {
    ...
    while (iter.hasNext) {
      val str = iter.next
      val data = ReformatDataParser.parseRequest(str, set)
      if (!data.isEmpty) {
        val arr = data.get
        ...
        CombineUtil.combineSC(arr, help_arr, 0, arr.length - 1, list)
