...
object StatUserDwelltime {
  ...
  def dwelltime(dwelltimeInput: String, sc: SparkContext): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
    ...
    val outputRdd = dwelltimeRdd.map(x => (x._1, ((0L, 0L), (0L, 0L), (0L, 0L), x._2)))
    outputRdd
  }
  ...
  def processPartition(iter: Iterator[String]): Iterator[(String, (Long, LinkedList[String]))] = {
    ...
    while (iter.hasNext) {
      ...
      if (!data.isEmpty) {
        ...
        val uid = arr.apply(8)
        if (arr.length >= 9) {
            var dwelltime = arr.apply(9).toLong
            if (dwelltime > 1000L) {
              dwelltime = 1000L
