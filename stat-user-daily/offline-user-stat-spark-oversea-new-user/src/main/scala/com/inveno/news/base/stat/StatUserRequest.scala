...
object StatUserRequest {
  ...
  private val CLASS_NAME = StatUserRequest.getClass.getSimpleName

  private val logger = LoggerFactory.getLogger(CLASS_NAME)

  def request(requestInput: String, sc: SparkContext, br: Broadcast[Map[String, String]]): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
    ...
    val processRdd = inputRdd.mapPartitions{iter => {
        ...
        while (iter.hasNext) {
          val str = iter.next
          val data = ReformatDataParser.parseRequest(str, profilemap)
          if (!data.isEmpty) {
            val arr = data.get
    ...
    val outputRdd = requestRdd.map(x => (x._1, (x._2, (0L, 0L), (0L, 0L), (0L, 0L), (0L, 0L))))
    outputRdd
  }
