...
object StatUserClick {
  ...
  def click(clickInput: String, sc: SparkContext, br: Broadcast[Map[String, String]]): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
    ...
    val processRdd = inputRdd.mapPartitions{iter => {
        ...
        while (iter.hasNext) {
          val str = iter.next
          val data = ReformatDataParser.parseClick(str, profilemap)
          if (!data.isEmpty) {
            val arr = data.get
