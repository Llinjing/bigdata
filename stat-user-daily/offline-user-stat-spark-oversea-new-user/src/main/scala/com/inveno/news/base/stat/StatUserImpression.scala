...
object StatUserImpression {
  ...
  def impression(impressionInput: String, sc: SparkContext, br: Broadcast[Map[String, String]]): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
    ...
    val processRdd = inputRdd.mapPartitions{iter => {
        ...
        while (iter.hasNext) {
          val str = iter.next
          val data = ReformatDataParser.parseImpression(str, profilemap)
          if (!data.isEmpty) {
            val arr = data.get
