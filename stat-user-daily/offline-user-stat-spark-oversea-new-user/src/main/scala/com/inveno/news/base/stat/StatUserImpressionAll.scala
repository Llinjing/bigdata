...
object StatUserImpressionAll {
  ...
  def impression(impressionInput: String, clickInput: String, sc: SparkContext, br: Broadcast[Map[String, String]]): RDD[(String, ((Long, Long), (Long, Long), (Long, Long), (Long, Long), (Long, Long)))] = {
    val impressionInputRdd = sc.textFile(impressionInput)
    val impressionNopushRdd = impressionInputRdd.mapPartitions{iter => {
        val map = new HashMap[String, (Long, LinkedList[String])]()

        var list = new java.util.ArrayList[String]()
        var help_arr = new Array[String](32)
        
        val profilemap = br.value
        
        while (iter.hasNext) {
          val str = iter.next
          val data = ReformatDataParser.parseImpressionNopush(str, profilemap)
          if (!data.isEmpty) {
            val arr = data.get
            val uid = arr.apply(arr.length - 1)
            list.clear()
          
            CombineUtil.combineImpression(arr, help_arr, 0, arr.length - 1, list)
    ...
    val clickPushRdd = clickInputRdd.mapPartitions{iter => {
        val map = new HashMap[String, (Long, LinkedList[String])]()
        ...
        while (iter.hasNext) {
          val str = iter.next
          val data = ReformatDataParser.parseClickPush(str, profilemap)
          if (!data.isEmpty) {
            val arr = data.get
            val uid = arr.apply(arr.length - 1)
            list.clear()
