...
object ReformatDataParser {
  ...
  def parseRequest(str: String): Option[Array[String]] = {
    ...
    try {
          ...
          val protocol = "https"
          val arr = Array(platform, product_id, app_ver, language, promotion, news_configid, protocol, content_type, strategy, scenario_channel, scenario, biz_configid, uid)
          if (isNotNull(arr)) {
            ret = Option(arr)
          }
    } catch {
  ...
  def parseDwelltime(str: String): Option[Array[String]] = {
    ...
    try {
          ...
          val uid = json.getString("uid")
          val protocol = "https"
          val arr = Array(platform, product_id, app_ver, language, promotion, news_configid, protocol, content_type, strategy, scenario_channel, scenario, biz_configid, uid, dwelltime)
          if (isNotNull(arr)) {
            ret = Option(arr)
          }
    } catch {
