 ...
 */
object StatAdRequest {

    private val CLASS_NAME = StatAdRequest.getClass.getSimpleName

    private val logger = LoggerFactory.getLogger(CLASS_NAME)
  
    def adRequest(input: String, sc: SparkContext): RDD[(String, (Long, Long, Long, Long, Long, Long, Long, Long, Long, Long, Long))] = {
        val inputRdd = sc.textFile(input)
        val processRdd = inputRdd.mapPartitions(processPartition)
        val statRdd = processRdd.map(x => (x._1, x._2)).reduceByKey(_ + _, 50).map(x => (x._1, (0L, 0L, 0L, 0L, 0L, 0L, x._2, 0L, 0L, 0L, 0L)))
        statRdd
    }
    ...
    def processPartition(iter: Iterator[String]): Iterator[(String, Long)] = {
      ...
      while (iter.hasNext) {
        val str = iter.next
        val data = ReformatDataParser.parseAdRequest(str)
        if (!data.isEmpty) {
          val arr = data.get
