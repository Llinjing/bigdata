...
function monitor()
{
	...
	if [ ${send} -eq ${get} ] ; then
		...
		# if job finish , should write date into kafka again
		if [ ${get} -gt 0 ] ; then
			sh -x ${LOCAL_BIN_PATH}/find_lost_data.sh
			${WARNING_BIN} "${PRODUCT_LINE}: ${APP_NAME[0]} data exceptions, last time send ${send} lines to kafka article-gmp topic, but current receive ${get} lines, will rewrite" "${SMS_USER[$INFO]}"
		else
			${WARNING_BIN} "${PRODUCT_LINE}: ${APP_NAME[0]} data exceptions, last time send ${send} lines to kafka article-gmp topic, but current receive ${get} lines" "${SMS_USER[$INFO]}"
		fi
	fi
}
