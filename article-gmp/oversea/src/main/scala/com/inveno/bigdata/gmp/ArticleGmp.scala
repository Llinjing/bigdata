...
object ArticleGmp {
  ...
  /**
   * input:
   * calculate GMP - (articleId###productId, (decay_click, total_impression, gmp))
   * output:
   * Json Data -
  ...
  def handler(ssc: StreamingContext) = {
    ...
    val click_stat = click_window.flatMap(msg => {
      ...
      List((key, 1L))
    }).reduceByKey(ReduceUtil.reduceAddLong _, 18).map(x => (x._1, (0f, x._2.toFloat, 0L, TimeUtil.getCurrDate(date_format), 1))).checkpoint(Minutes(window_size))
    ...
    val impression_stat = impression_window.flatMap(msg => {
      ...
      val key = CombinationKey.makeArticleCombinationKey(key_tag, key_arr(0), key_arr(1))
      List((key, 1L))
    }).reduceByKey(ReduceUtil.reduceAddLong _, 18).map(x => (x._1, (x._2.toFloat, 0f, x._2, TimeUtil.getCurrDate(date_format), 1))).checkpoint(Minutes(window_size))
    ...
    val history_gmp_stat = history_gmp_window.flatMap(msg => {
      //content_id, product_id, decay_impression, decay_click, total_impression, update_date, isValid
      ...
      val decay_click = key_arr(3).toFloat
    ...
    // current data - (articleId###productId, (decay_impression, decay_click, total_impression, update_date, click_impression_tag))
    ...
    try {
      ...
      if (is_debug) {
        ...
        //history_gmp_stat.saveAsTextFiles(hdfs_debug + "/history-gmp-stat")
        //current_feedback_result.saveAsTextFiles(hdfs_debug + "/current-feedback-result")
      }
    } catch {
      case ex: Exception => logger.warn(ex.getMessage, ex)
    }
