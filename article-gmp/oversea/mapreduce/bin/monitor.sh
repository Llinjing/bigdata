...
function check_blocked_task()
{
	...
	then
		${WARNING_BIN} "${PRODUCT_LINE}_:_article-gmp_running_task_blocked_:_${task_num}" ${SMS_USER[$FATAL]}
	fi

        task_num=`ls ${LOCAL_WAITING_RUNNING_TASK_POOL_PATH} | wc -l`
        if [ $task_num -gt ${BLOCKED_TASK_NUM} ]
        then
                ${WARNING_BIN} "${PRODUCT_LINE}_:_article-gmp_waiting_running_task_blocked_:_${task_num}" ${SMS_USER[$FATAL]}
        fi
	return 0
}
