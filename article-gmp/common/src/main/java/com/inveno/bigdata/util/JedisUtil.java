package com.inveno.bigdata.util;
...
public class JedisUtil {
	...
	public static JedisCluster createJedisCluster(String redis_servers) {
		JedisCluster jedis_cluster = null;
		
		try {
			Set<HostAndPort> nodes = new HashSet<HostAndPort>();
		
			for (Pair<String, Integer> pair : ServerURI.parse(redis_servers)) {
				nodes.add(new HostAndPort(pair.first, pair.second));
			}
