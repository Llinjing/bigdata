 ...
 object StatUserRequest {
   ...
   def request(requestInput: String, sc: SparkContext, br: Broadcast[Map[String, String]]): RDD[(String, ((Long, Long), (Long, Long), (Long, Long)))] = {
     ...
     val statRdd = processRdd.map(x => (x._1, x._2._1)).reduceByKey(_ + _, 200).map(x => (x._1, (x._2, 0L)))
 
     val statUserRdd = processRdd.mapPartitions(userPartition)
       .reduceByKey((a, b) => a.append(b).distinct)
       .mapPartitions(statPartition)
       .reduceByKey(_ + _, 500).map(x => (x._1, (0L, x._2)))
 
     val requestRdd = statRdd.union(statUserRdd).reduceByKey((a, b) => (a._1 + b._1, a._2 + b._2))
 
     val outputRdd = requestRdd.map(x => (x._1, (x._2, (0L, 0L), (0L, 0L))))
     outputRdd
   }
   
   def requestTmpPartition(iter: Iterator[String]): Iterator[(String, String)] = {
     var ret_list = List[(String, String)]()
