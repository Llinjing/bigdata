...
object MergeUserStatInfo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val articleInfoRdd = StatArticleInfo.getArticleInfo(args.apply(0), args.apply(1), args.apply(2), sc).collectAsMap
    
    val broadcastRdd1 = sc.broadcast(articleInfoRdd)
    val requestRdd = StatUserRequest.request(args.apply(3), sc, broadcastRdd1)
    
    val broadcastRdd2 = sc.broadcast(articleInfoRdd)
    val impressionRdd = StatUserImpression.impression(args.apply(4), sc, broadcastRdd2)
    
    val broadcastRdd3 = sc.broadcast(articleInfoRdd)
    val clickRdd = StatUserClick.click(args.apply(5), sc, broadcastRdd3)
    
    val broadcastRdd4 = sc.broadcast(articleInfoRdd)
    val dwelltimeRdd = StatUserDwelltime.dwelltime(args.apply(6), sc, broadcastRdd4)
    ...
    ressultRdd.saveAsTextFile(args.apply(7))
