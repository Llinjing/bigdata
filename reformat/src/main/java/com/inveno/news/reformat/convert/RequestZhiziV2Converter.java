...
public class RequestZhiziV2Converter extends AbstractConverter {

	private static final Logger logger = LoggerFactory.getLogger(RequestZhiziV2Converter.class);
	...
	@Override
	public Object clone() {
		return new RequestZhiziV2Converter();
	}
	...
	private CommonSchema parseRequestExtra(String [] arr, int configid_col, JSONObject json_obj, int order_num) {		
		...
		try {
			...
			SpecialFieldsFiller.fillUpack(configid_json_obj.getString("upack"), schema, abDyeingClient);
		} catch (Exception e) {
			logger.warn("no upack data in log");
			SpecialFieldsFiller.fillUpack("", schema, abDyeingClient);
		}
