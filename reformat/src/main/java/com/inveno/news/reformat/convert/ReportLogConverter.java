...
public class ReportLogConverter extends AbstractConverter {
	
	private static final Logger logger = LoggerFactory.getLogger(ReportLogConverter.class);
	...
	private CommonSchema parseExtra(String [] arr, JSONObject json_obj, String product_id) {
		...
		try {
			JSONObject obj = JSONObject.parseObject(new String(base64.decode(arr[28])));
	...
	private void parseArticleImpressionExtra(JSONObject json_obj, CommonSchema schema, String product_id) {
		...
		try {
			String cpack_decode_str = new String(base64.decode(json_obj.getString("cpack")));
