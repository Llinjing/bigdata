...
public class UadLogConverter extends AbstractConverter {
	
	private static final Logger logger = LoggerFactory.getLogger(RequestLogConverter.class);
	
	public static final String LOG_TAG = "uad";
	
	public static final int LOG_MIN_COLUMNS = 20;
	...
	public Map<String, List<String>> convert(String str) {
		...
		if (str.startsWith(LOG_TAG)) {	//prefix: uad or uad_s
			String [] arr_raw = str.split("&");
			String [] arr = ArrayUtil.expand(arr_raw, LOG_MIN_COLUMNS, "");
			if (arr != null && arr.length >= LOG_MIN_COLUMNS) {
				if (!MessageValidator.validate(arr[3], arr[12], arr[5], str)) {
					return map;
				}
				
				if (ProductID.MEIZU.equals(arr[12])) {
					logger.warn("ignore meizu");
					return map;
				}
				
				String content_type = ContentIDHelper.getContentType(arr[5]);
				if (ProductID.ALI.equals(arr[12]) && !content_type.startsWith("advertisement")) {
				...
				boolean is_recomm = arr[6].contentEquals("2");
			
				// pos只要有值，就是相关推荐
				SpecialFieldsFiller.fillSpecialFields(arr[0], arr[12], "", is_recomm, schema);
				
				this.fillUadClickExtra(arr, arr_raw.length, schema);
	...
	private void fillUadClickExtra(String [] arr, int configid_col, CommonSchema schema) {
		ArticleClickExtra extra = new ArticleClickExtra();
		extra.setContent_id(arr[5]);
		extra.setContent_type(ContentIDHelper.getContentType(arr[5]));
		...
		try {
			...
			String app_ver = json_obj.getString("app_ver");
			if (!StringUtil.isEmpty(app_ver)) {
				schema.setApp_ver(app_ver);
			}
